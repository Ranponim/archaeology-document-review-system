...
async def test_corpus_mode_runs_linker_then_graph_rules_and_emits_one_pending_candidate():
    ...
    orchestrator = _orchestrator(
        events=events,
        graph_engine=FakeGraphRuleEngine(events),
    )
...
@pytest.mark.anyio
async def test_optional_failure_warning_preserves_all_graph_candidates():
    events = []
    dispatcher = FakeOptionalDispatcher(warnings=["optional ai review warning: TimeoutError"])
    orchestrator = _orchestrator(
        events=events,
        graph_engine=FakeGraphRuleEngine(events, include_semantic=True),
        optional_dispatcher=dispatcher,
    ...
    assert len(call["findings"]) == 2
    assert len(result.candidates) == 2
    assert result.status == "completed"
    assert result.warnings == ["optional ai review warning: TimeoutError"]
    assert result.summary["optional_review_findings"] == []
