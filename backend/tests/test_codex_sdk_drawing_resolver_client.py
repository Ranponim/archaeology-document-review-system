...
class _FakeThread:
    ...
    def run(self, inputs, **kwargs):
        self.run_calls.append((inputs, kwargs))
        return SimpleNamespace(id=f"turn-{len(self.run_calls)}", final_response=self.responses.pop(0))


class _FakeCodex:
    def __init__(self, responses: list[str]) -> None:
        self.thread = _FakeThread(responses)
...
def test_sdk_client_reuses_codex_session_without_openai_api_key(monkeypatch, tmp_path):
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    source = _source(tmp_path)
    candidate = _candidate(tmp_path)
    fake = _FakeCodex([json.dumps(_decision(candidate), ensure_ascii=False)])
    ...
    inputs, kwargs = fake.thread.run_calls[0]
...
def test_sdk_client_keeps_closed_world_validation_and_retries_once(tmp_path):
    ...
    fake = _FakeCodex([invented, invented])
    workdir = tmp_path / "codex-work"
    workdir.mkdir()
    client = CodexSdkDrawingResolverClient(
        codex_client=fake,
