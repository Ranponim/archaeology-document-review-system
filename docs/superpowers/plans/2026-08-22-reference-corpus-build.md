...
**Architecture:** Existing `OriginalAsset` storage remains the provenance layer. A new `ReferenceCorpus` aggregate stages source assets, delegates structural extraction to an Adobe-converter boundary, validates a versioned manifest, canonicalizes explicit internal publication identifiers, and persists corpus-scoped `Plate/PlatePanel/Drawing/DrawingRegion` nodes to Neo4j. GitHub CI uses fake converter manifests; real Adobe smoke tests are a separate self-hosted gate.

**Tech Stack:** Python 3.12, FastAPI, Neo4j 5.26, RQ/Redis, dataclasses/Pydantic-style validation used by the existing backend, React/TypeScript/Vitest for the UI.
...
- Create `backend/app/domain/adobe_manifest.py`: normalized manifest contract consumed by Python.
...
- Create `backend/app/services/adobe_conversion_client.py`: converter port plus deterministic fake/in-process adapter contract used by tests.
- Modify `backend/app/services/source_import_service.py`: expose uploaded source-role classification without allowing names to become identity.
...
- Modify `backend/app/main.py`: wire the new router/service/repository.
- Modify `backend/app/graph/schema.py`: constraints/indexes for `ReferenceCorpus` and `DerivedArtifact`.
- Modify `backend/app/domain/canonical_models.py`: add optional `reference_corpus_id` to visual canonical data objects.
- Modify `backend/app/graph/canonical_repository.py`: persist corpus-scoped membership instead of requiring visual `DocumentVersion` ownership for new paths.
...
- Test with `backend/tests/test_reference_corpus_service.py`, `backend/tests/test_reference_canonicalizer.py`, `backend/tests/test_reference_corpora_api.py`, `backend/tests/integration/test_reference_corpus_real_neo4j.py`, `frontend/src/referenceCorpusApi.test.ts`, and `frontend/src/components/ReferenceCorpusPanel.test.tsx`.
...
**Interfaces:**
- Produces: `ReferenceCorpusStatus`, `ReferenceCorpusFailureCode`, `ReferenceCorpusData`, `DerivedArtifactData`, and `compute_build_identity(...)`.
- Consumers: repository/service tasks below.
...
Add frozen dataclasses with the exact spec properties and SHA-256 build-identity calculation over the four ordered identity inputs.

- [ ] **Step 4: Add schema constraints/indexes**

Add `reference_corpus_id_unique`, `derived_artifact_id_unique`, and project/status/build-identity indexes to `backend/app/graph/schema.py`.

- [ ] **Step 5: Run GREEN**

Run: `cd backend && pytest -q tests/test_reference_corpus_domain.py tests/test_schema.py`
Expected: PASS.
...
**Interfaces:**
- Produces: `ReferenceCorpusRepository.create_staging`, `attach_source`, `save_artifact`, `transition_status`, `find_ready_by_build_identity`, `save_canonical_visuals`, `validate_ready_graph`.
- Consumes: Task 1 domain objects.

- [ ] **Step 1: Write repository contract tests for project scoping and immutability**

```python
def test_ready_corpus_rejects_new_source(repository, ready_corpus, source):
    with pytest.raises(ValueError, match="immutable"):
        repository.attach_source(ready_corpus.id, source.id)


def test_cross_project_source_is_rejected(repository, corpus, foreign_source):
    with pytest.raises(ValueError, match="project"):
        repository.attach_source(corpus.id, foreign_source.id)
```
...
- [ ] **Step 3: Implement scoped Cypher only inside the repository**

Use `MATCH (p:Project {id:$project_id})-[:HAS_REFERENCE_CORPUS]->(c:ReferenceCorpus {id:$corpus_id})` as the root for every mutation. `transition_status` must reject any mutation from `ready` except read operations. `save_canonical_visuals` must create `HAS_PLATE/HAS_DRAWING` from the corpus and preserve `HAS_PANEL/HAS_REGION` underneath canonical nodes.

- [ ] **Step 4: Add corpus identity to visual domain params**

Add `reference_corpus_id: str | None = None` to `PlateData` and `DrawingData`. New canonical persistence must use corpus membership when present; the old `DocumentVersion` relationship stays only for legacy compatibility.

- [ ] **Step 5: Run unit and real Neo4j GREEN**

Run: `cd backend && pytest -q tests/test_reference_corpus_repository.py tests/integration/test_reference_corpus_real_neo4j.py`
Expected: PASS, including READY mutation rejection and cross-project isolation.
...
### Task 3: Versioned Adobe manifest and converter boundary
...
**Interfaces:**
- Produces: `AdobeManifestV1.from_dict`, `ConversionRequest`, `ConversionResult`, `AdobeConversionClient.convert` protocol.
- Consumers: canonicalizer and corpus service.
...
Run: `cd backend && pytest -q tests/test_adobe_manifest.py tests/test_adobe_conversion_client.py`
Expected: FAIL because manifest/converter types do not exist.

- [ ] **Step 3: Implement normalized manifest dataclasses and validation**

Model only the structural fields needed by the spec: pages/artboards, text frames, bounds, graphic/placed items, link IDs/paths, object IDs, application/version, and artifact descriptors. Do not parse publication numbers here.

- [ ] **Step 4: Implement converter port and fake adapter**
...
The production adapter may call an external agent later; ordinary CI must use a deterministic fake returning fixture manifests and artifact bytes/metadata.
...
```bash
git add backend/app/domain/adobe_manifest.py backend/app/services/adobe_conversion_client.py backend/tests/test_adobe_manifest.py backend/tests/test_adobe_conversion_client.py backend/tests/fixtures/reference_corpus
git commit -m "feat(source): define adobe conversion contract"
```
...
- Consumes: Adobe manifest types and `OriginalAssetData`.

- [ ] **Step 1: Write RED tests for the authority rules**
...
- [ ] **Step 3: Implement explicit-identifier parsing only over manifest text**

Use explicit patterns such as `【도판 45】` and `【도면 30】` from manifest text frames. Generate corpus-scoped IDs exactly as defined in the spec. Use InDesign graphic-frame/link-ID placement for panel provenance. Never inspect the source filename when selecting canonical number or visual target.

- [ ] **Step 4: Implement fail-closed error set**

Return/raise exact codes for `LINK_MISSING`, `IDENTIFIER_UNRESOLVED`, `AMBIGUOUS_IDENTIFIER`, `DUPLICATE_CANONICAL_IDENTIFIER`, and `PROVENANCE_INCOMPLETE`. DrawingRegion creation requires deterministic region ownership; otherwise keep the parent Drawing without inventing regions.
...
- Consumes: Tasks 2-4 repositories/converter/canonicalizer.
...
- [ ] **Step 3: Implement the state machine exactly**

`staging -> converting -> validating -> canonicalizing -> graph_validating -> ready`. On exception, persist `failed` plus the normalized failure code while retaining source/artifact diagnostics. Compute `sourceSetHash` from sorted source asset SHA-256 values plus roles, then derive the spec build identity.

- [ ] **Step 4: Keep source role separate from identity**

Extend `SourceImportService`/source staging so `.indd`, linked images, and `.ai` are accepted with explicit roles. Role determines which converter input bucket receives the source; it never provides publication number identity.
...
```bash
git add backend/app/services/reference_corpus_service.py backend/app/services/source_import_service.py backend/tests/test_reference_corpus_service.py
...
**Interfaces:**
- Produces the spec endpoints and typed response payloads.
- Consumes: `ReferenceCorpusService`.
...
def test_new_plate_pdf_upload_is_rejected(client, project_id):
    response = upload_document(client, project_id, "plate_book", "plates.pdf")
...
- [ ] **Step 3: Implement create/upload/build/list/detail endpoints**

Accept only roles/extensions in the spec. Build returns accepted/build status, not a false READY result. Include corpus revision/status/counts/failure diagnostics in detail response.

- [ ] **Step 4: Restrict the normal document endpoint for new visual uploads**

Keep body PDF upload supported. Preserve legacy stored versions/read paths but reject creating new `plate_book`/`drawing_book` publication-PDF authority through the new UI/API path.
...
```bash
git add backend/app/api/reference_corpora.py backend/app/main.py backend/app/api/schemas.py backend/tests/test_reference_corpora_api.py
...
**Interfaces:**
- Produces a separate reference-data panel and API types.
...
- [ ] **Step 3: Implement typed API calls and panel**

Show current corpus revision/status, INDD source, linked-photo count/status, AI count/status, extracted canonical counts, failure diagnostics, and build/rebuild action. Keep the body PDF control separate in the review area.

- [ ] **Step 4: Remove new visual-PDF selection from ProjectDetailPage**

The existing `uploadKind` UI may remain internally only for legacy rendering if needed, but users must not be offered new plate/drawing PDF authority. Body upload label must read `본문 PDF 선택`.
...
### Task 8: ReferenceCorpus CI/E2E gate

**Files:**
- Modify: `.github/workflows/review-remediation-ci.yml` if required by current workflow layout.
- Test: `backend/tests/integration/test_reference_corpus_real_neo4j.py`
- Add: `backend/tests/test_reference_corpus_build_e2e.py`

**Interfaces:**
- Produces fresh authoritative CI evidence for Plan A.

- [ ] **Step 1: Add fixture-converter E2E test**

Exercise project creation -> corpus create -> INDD/Links/AI staging -> fake Adobe conversion -> canonicalization -> real Neo4j READY graph. Assert corpus-scoped IDs, provenance edges, counts, and filename non-authority.

- [ ] **Step 2: Run focused backend suite**

Run: `cd backend && pytest -q tests/test_reference_corpus_domain.py tests/test_adobe_manifest.py tests/test_adobe_conversion_client.py tests/test_reference_canonicalizer.py tests/test_reference_corpus_service.py tests/test_reference_corpora_api.py tests/test_reference_corpus_build_e2e.py`
Expected: PASS.

- [ ] **Step 3: Run real Neo4j integration suite**
...
- [ ] **Step 5: Commit any CI-only changes**

```bash
git add .github/workflows backend/tests frontend/src
